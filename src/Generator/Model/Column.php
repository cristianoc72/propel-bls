...
class Column
{
    use DomainPart, NamePart, GeneratorPart, TablePart, PlatformAccessorPart, DescriptionPart, VendorPart;

    const CONSTANT_PREFIX    = 'COLUMN_';
     ...
     * @return Table
     */
    protected function getSuperordinate(): ?Table
    {
        return $this->table;
    }
    ...
    {
        return "{$this->getTable()->getName()}.{$this->getName()->toUpperCase()}";
    }

    /**
     * Facility method for builders
     *
     * @return Text
     */
    ...
    {
        if (!$this->singularName->isEmpty()) {
            return $this->singularName;
        }

        return $this->getName()->toSingular();
    }
    ...
    /**
     * Returns the full column constant name (e.g. TableMapName::COLUMN_COLUMN_NAME).
     *
     * @return string A column constant name for insertion into PHP code
     */
    public function getFQConstantName(): string
    {
        $classname = $this->getTable()->getName() . 'TableMap';
    ...
    {
        return self::CONSTANT_PREFIX . $this->getName()->toSnakeCase()->toUpperCase()->toString();
    }
    ...
    {
        return $this->getPlatform()->getNullString(true);
    }
    ...
    {
        return $this->getForeignKeys()->size() > 0;
    }
    ...
    {
        return $this->getForeignKeys()->size() > 1;
    }
     ...
     *
     * @param  ColumnDefaultValue|string $defaultExpression The column's default value
     */
    public function setDefaultExpression($defaultExpression): void
    {
        if (!$defaultExpression instanceof ColumnDefaultValue) {
            $defaultExpression = new ColumnDefaultValue($defaultExpression, ColumnDefaultValue::TYPE_EXPR);
        }
    ...
    {
        if ($this->isAutoIncrement() && Model::ID_METHOD_NATIVE === $this->getTable()->getIdMethod()) {
